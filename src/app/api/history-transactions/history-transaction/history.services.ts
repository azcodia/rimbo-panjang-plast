...
export const createHistoryTransaction = async (data: {
  ...
  // 🔥 HARGA HANYA UNTUK OUT
  if (data.type === "out" && data.unit_price != null) {
    payload.unit_price = data.unit_price;
    payload.total_price = data.quantity * data.unit_price;
  }
...
export const updateHistoryTransactionByToken = async (
  ...
  if (data.type === "out" && data.unit_price != null && data.quantity != null) {
    ...
    updateData.total_price = data.quantity * data.unit_price;
  }
